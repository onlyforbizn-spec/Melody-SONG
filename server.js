...
function generateLyricsPDF(outPath, recipientName, lyrics) {
  return new Promise((resolve, reject) => {
    ...
    const IVORY = '#FAF8F3';
    const NAVY = '#1B2A4A';
    const NAVY_SOFT = '#5B6B85';
    const GOLD = '#C9A227';
    const pageW = doc.page.width;
    const pageH = doc.page.height;

    doc.rect(0, 0, pageW, pageH).fill(IVORY);
    ...
    let y = M + 60;
    const ornY = y;
    doc.lineWidth(0.6).strokeColor(NAVY_SOFT);
    const cx = pageW / 2;
    doc.fillColor(NAVY).moveTo(cx, ornY - 4).lineTo(cx + 4, ornY).lineTo(cx, ornY + 4).lineTo(cx - 4, ornY).closePath().fill();
    doc.strokeColor(NAVY).moveTo(cx - 18, ornY - 3).lineTo(cx - 15, ornY).lineTo(cx - 18, ornY + 3).lineTo(cx - 21, ornY).closePath().stroke();
    ...
    doc.font('Times-Italic').fontSize(42).fillColor(NAVY)
      .text('Melody Song', M, y, { width: pageW - 2 * M, align: 'center' });
    y += 52;
    ...
    doc.fillColor(NAVY).opacity(0.06)
    ...
    doc.fillColor(NAVY).font('Times-Italic').fontSize(18)
    ...
    doc.moveTo(waveCx - 40, waveY)
      ...
      .stroke();
    doc.fillColor(NAVY).moveTo(waveCx, waveY - 3).lineTo(waveCx + 3, waveY).lineTo(waveCx, waveY + 3).lineTo(waveCx - 3, waveY).closePath().fill();

    doc.fillColor(NAVY_SOFT).font('Times-Italic').fontSize(9)
      .text('Made with love by the Melody Song team', M, footerY + 14,
        { width: pageW - 2 * M, align: 'center' });
    doc.fillColor(NAVY).font('Times-Roman').fontSize(9)
      .text('melody-song.com', M, footerY + 26,
        { width: pageW - 2 * M, align: 'center' });
...
app.get('/', (req, res) => res.send('Melody Song audio server OK'));
